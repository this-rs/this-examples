...
async fn bench_single_get_endpoint_dynamo(server: &TestServer, endpoint: &str) -> anyhow::Result<()> {
...
async fn bench_single_post_endpoint_dynamo(server: &TestServer, endpoint: &str, data: serde_json::Value) -> anyhow::Result<()> {
...
async fn stress_test_dynamo_rest(server: &TestServer, concurrent_requests: usize) -> anyhow::Result<Vec<u128>> {
    ...
    println!("🗄️ DynamoDB - {} requêtes concurrentes en {:?}", concurrent_requests, total_time);
    Ok(latencies?)
}
        ...
        group.bench_with_input(
            ...
            |b, (server, endpoint)| {
                b.to_async(&rt).iter(|| async {
                    bench_single_get_endpoint_dynamo(server, endpoint).await.unwrap()
                });
            },
        );
    }
    ...
    for (name, endpoint, sample_data) in post_test_cases {
        group.bench_with_input(
            ...
            |b, (server, endpoint, data)| {
                b.to_async(&rt).iter(|| async {
                    bench_single_post_endpoint_dynamo(server, endpoint, data.clone()).await.unwrap()
                });
            },
        );
    }
...
fn benchmark_graphql_dynamodb(c: &mut Criterion) {
    ...
    let server_result = rt.block_on(async {
        match create_graphql_server_dynamodb().await {
            Ok(router) => start_test_server(router).await,
            Err(e) => {
                println!("⚠️ DynamoDB non disponible pour GraphQL, benchmark ignoré: {}", e);
                return Err(e);
            }
        }
    });
    ...
    let query_test_cases = vec![
        ("orders_query", json!({"query": "query { orders { id name number amount status } }"})),
        ("invoices_query", json!({"query": "query { invoices { id name number amount status } }"})),
        ("payments_query", json!({"query": "query { payments { id name number amount status } }"})),
    ];
    ...
    for (name, mutation) in mutation_test_cases {
        group.bench_with_input(
            ...
            |b, (server, mutation)| {
                b.to_async(&rt).iter(|| async {
                    let response = server.post_json("/graphql", mutation.clone()).await.unwrap();
                    let body = TestServer::response_body_string(response).await.unwrap();
                    black_box(body);
                });
            },
...
fn benchmark_dynamodb_load_test(c: &mut Criterion) {
    ...
    let server_result = rt.block_on(async {
        match create_rest_server_dynamodb().await {
            Ok(router) => start_test_server(router).await,
            Err(e) => {
                println!("⚠️ DynamoDB non disponible pour load test, benchmark ignoré: {}", e);
                return Err(e);
            }
        }
    });
    ...
    for concurrent_req in [1, 5, 10, 20].iter() {
        group.bench_with_input(
            ...
            |b, &concurrent_req| {
                b.to_async(&rt).iter(|| async {
                    let latencies = stress_test_dynamo_rest(&server, concurrent_req).await.unwrap();
...
fn benchmark_comparison(c: &mut Criterion) {
    ...
    let (in_memory_server, dynamo_server_opt) = rt.block_on(async {
        let in_memory_router = endpoint_benchmarks::create_rest_server_in_memory().await.unwrap();
    ...
    group.bench_function("get_orders_in_memory", |b| {
        b.to_async(&rt).iter(|| async {
            bench_single_get_endpoint_dynamo(&in_memory_server, "/orders").await.unwrap()
        });
    });

    group.bench_function("get_orders_dynamodb", |b| {
        b.to_async(&rt).iter(|| async {
            bench_single_get_endpoint_dynamo(&dynamo_server, "/orders").await.unwrap()
        });
    });
    ...
    group.bench_function("post_orders_in_memory", |b| {
        b.to_async(&rt).iter(|| async {
            bench_single_post_endpoint_dynamo(&in_memory_server, "/orders", data::sample_order()).await.unwrap()
        });
    });

    group.bench_function("post_orders_dynamodb", |b| {
        b.to_async(&rt).iter(|| async {
            bench_single_post_endpoint_dynamo(&dynamo_server, "/orders", data::sample_order()).await.unwrap()
        });
    });
