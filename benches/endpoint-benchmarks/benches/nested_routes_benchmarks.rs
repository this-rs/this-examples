...
async fn benchmark_nested_routes_with_real_ids(
    server: &TestServer,
) -> anyhow::Result<Vec<NestedRouteResult>> {
    ...
    if let Some(order_id) = order_id {
        ...
        if let Some(invoice_id) = invoice_id {
            ...
            println!(
                "\n📊 Niveau 4 : /orders/{}/invoices/{}/payments",
                order_id, invoice_id
            );
            ...
            if let Some(payment_id) = payment_id {
                ...
                println!(
                    "\n📊 Niveau 5 : /orders/{}/invoices/{}/payments/{}",
    ...
    for result in &results {
        let overhead = if result.depth == 0 {
            0
        } else {
            result.response_time - results[0].response_time
        };
        println!(
            "Niveau {} | {}µs | Overhead: {}µs | Route: {}",
...
fn benchmark_nested_routes_performance(c: &mut Criterion) {
    ...
    group.bench_function("full_nested_route_traversal", |b| {
        b.to_async(&rt).iter(|| async {
            let results = benchmark_nested_routes_with_real_ids(black_box(&server))
                .await
                .unwrap();
            black_box(results)
        });
...
fn benchmark_nested_routes_scaling(c: &mut Criterion) {
    ...
    let (order_id, invoice_id, payment_id) = rt.block_on(async {
        ...
        let response = server
            .get(&format!("/orders/{}/invoices", order_id))
            .await
            .unwrap();
        let body = TestServer::response_body_string(response).await.unwrap();
    ...
    let routes = vec![
        ...
        (
            "level_2_order_invoices",
            format!("/orders/{}/invoices", order_id),
            2u8,
        ),
        (
            "level_3_specific_invoice",
            format!("/orders/{}/invoices/{}", order_id, invoice_id),
            3u8,
        ),
        (
            "level_4_invoice_payments",
            format!("/orders/{}/invoices/{}/payments", order_id, invoice_id),
            4u8,
        ),
        (
            "level_5_specific_payment",
            format!(
                "/orders/{}/invoices/{}/payments/{}",
                order_id, invoice_id, payment_id
            ),
