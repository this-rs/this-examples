...
#[derive(Clone)]
pub struct InvoiceState { pub store: Arc<dyn InvoiceStore> }
...
pub async fn get_invoice(State(state): State<InvoiceState>, Path(id): Path<Uuid>) -> Json<Option<Invoice>> {
    let item = state.store.get(&id).await.ok();
    Json(item)
}

pub async fn create_invoice(State(state): State<InvoiceState>, Json(invoice): Json<Invoice>) -> Json<Invoice> {
    let created = state.store.create(invoice.clone()).await.unwrap_or(invoice);
    Json(created)
}

pub async fn update_invoice(State(state): State<InvoiceState>, Json(invoice): Json<Invoice>) -> Json<Option<Invoice>> {
