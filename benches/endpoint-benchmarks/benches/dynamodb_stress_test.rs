            ...
            match result {
                Ok(response) => {
                    ...
                    if status.is_success() {
                        ...
                        println!("⚠️ Erreur HTTP {}: {}", status, endpoint);
                ...
                Err(e) => {
                    ...
                    if is_timeout {
                        println!("⏰ Timeout sur {}: {}", endpoint, e);
                    } else {
                        println!("❌ Erreur sur {}: {}", endpoint, e);
                    }
...
impl StressTestResults {
    fn print_stats(&self, test_name: &str) {
        ...
        if len == 0 {
            println!("❌ Aucune mesure valide pour {}", test_name);
            return;
        }
        ...
        if p95 as f64 / 1000.0 < 100.0 {
            ...
            println!("👍 Performance acceptable (P95 < 500ms)");
        } else {
            println!("⚠️  Performance dégradée (P95 > 500ms)");
        }
    }
}
...
fn dynamodb_stress_test_scaling(c: &mut Criterion) {
    ...
    let server = match server_result {
        Ok(s) => s,
        Err(_) => {
            println!("⏭️  Skipping DynamoDB stress tests - DynamoDB non disponible");
            return;
        }
    };
