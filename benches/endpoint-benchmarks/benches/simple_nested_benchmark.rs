...
fn benchmark_nested_routes_simple(c: &mut Criterion) {
    ...
    let (order_id, invoice_id) = rt.block_on(async {
        ...
        let invoice_id = invoices["data"][0]["target"]["id"]
            .as_str()
            .unwrap()
            .to_string();
    ...
    for (name, route) in routes {
        group.bench_with_input(BenchmarkId::new("route", name), &route, |b, route| {
            ...
            b.to_async(&rt).iter(|| {
                ...
                async move {
                    ...
                    let depth = if route == "/orders" {
                        0
                    } else if route.contains("/invoices/") {
                        3
                    } else if route.contains("/invoices") {
                        2
                    } else {
                ...
                |b, (route, concurrent)| {
                    ...
                    b.to_async(&rt).iter(|| {
                        ...
                        async move {
                            ...
                            println!(
                                "🚀 Niveau {} | {} concurrent | {}µs total | {:.1} req/s",
                                depth,
                                concurrent,
